...
export const getDriverStock = async (
  driverId: string,
  date: Date = new Date(),
) => {
  ...
  const todayBuy = transactions
    .filter(
      (t) =>
        t.type === "BUY" ||
        t.type === "SHOP_BUY" ||
        (t.type === "PALTI" && t.paltiAction === "ADD"),
    )
  ...
  const todaySell = transactions
    .filter(
      (t) =>
        t.type === "SELL" ||
        (t.type === "PALTI" && t.paltiAction === "SUBTRACT"),
    )
    .reduce((sum, t) => sum + Number(t.amount), 0);
...
export const getDashboardStats = async (
  ...
  const totalStockIn = allTransactions
    .filter(
      (t) =>
        t.type === "BUY" ||
        t.type === "SHOP_BUY" ||
  ...
  const totalStockOut = allTransactions
    .filter(
      (t) =>
        t.type === "SELL" ||
        (t.type === "PALTI" && t.paltiAction === "SUBTRACT"),
    ...
    .reduce((sum, t) => sum + Number(t.amount || 0), 0);
  const totalWeightLoss = allTransactions
    .filter((t) => t.type === "WEIGHT_LOSS")
    .reduce((sum, t) => sum + Number(t.amount || 0), 0);
